    ...
    extract($_POST);
        ...
        if(isset($upload))
        {
            ...
            elseif($p==$pass)
            {
                ...
                $file_store="upload1/".$file_name;
                
                if(move_uploaded_file($file_tem_Loc, $file_store)){
                $a="LDCO";
                $q="INSERT INTO video(vname,sub) VALUES ('$file_name','$a')";
...
<head>
	...
	<tr>
		...
		<td>
                <?php
                    $q2="SELECT  vname FROM video";
                ?>
           <a href="upload1/<?php echo $files[$i] ?>"><video controls="" width="150px" height="80px" src="upload/<?php echo $files[$i] ?>"></video></a>
            
            <a href="upload1/<?php echo $files[$i] ?>"><?php echo $files[$i] ?></a>
            </td>
            <td>
            <a href="deletevideo1.php?fname='<?php echo $files[$i] ?>'"><button style="background-color:red; border-color:green; border-width:4px; color:white;">Delete</button></a>
		<?php  
		} 
        ?>
        </tr>
